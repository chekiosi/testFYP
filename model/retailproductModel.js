...
var retailProductDB = {
                        ...
                        if (err) {
                            ...
                            if(result.affectedRows > 0) {
                                if(imgPath != null && imgPath != '') {
                                    retailProductDB.updateProductImage(id, imgPath)
                                        .then((result) => {
    ...
    updateProductImage: function (id, imgPath) {
        return new Promise( ( resolve, reject ) => {
            var conn = db.getConnection();
            conn.connect(function (err) {
                if (err) {
                    ...
                    var sql = 'UPDATE retailproductentity SET IMAGEURL=? WHERE ID=?';
                    conn.query(sql, [imgPath, id], function (err, result) {
                        if (err) {
                            console.log(err);
                            return reject(err);
                        } else {
